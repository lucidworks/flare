# -*- encoding : utf-8 -*-
class SearchHistoryController < CatalogController
  ...
  def clear
    if session[:history].clear
      flash[:notice] = I18n.t('flare.search_history.clear.success')
    else
      flash[:error] = I18n.t('flare.search_history.clear.failure')
    end
    redirect_to :back
  end
end